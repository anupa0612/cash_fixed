...
user_manager = UserManager(mongo_handler)
    ...
    if request.method == "POST":
        ...
        if user and user.check_password(password):
            ...
            return render_template('login.html', 
                                 ...
                                 show_default_credentials=not user_manager.user_exists())
    
    # Show default credentials if no users exist
    show_default = not user_manager.user_exists()
    return render_template('login.html', show_default_credentials=show_default)
...
def admin_users():
    ...
    if request.method == "POST":
        ...
        if not username or not email or not password:
            ...
            success, result = user_manager.create_user(username, email, password, role)
            if success:
...
def change_password():
    ...
    if len(new_password) < 6:
        return redirect(url_for('admin_users', error='New password must be at least 6 characters'))
    
    # Verify current password
    user = user_manager.get_user_by_id(current_user.get_id())
    if not user or not user.check_password(current_password):
        return redirect(url_for('admin_users', error='Current password is incorrect'))
    
    # Update password
    if user_manager.update_user(current_user.get_id(), password=new_password):
        return redirect(url_for('admin_users', message='Password changed successfully'))
    else:
        return redirect(url_for('admin_users', error='Failed to update password'))
